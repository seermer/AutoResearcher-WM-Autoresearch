...
def arxiv_search(query: str, max_results: int = 8) -> str:
    ...
    try:
        r = httpx.get("http://export.arxiv.org/api/query", timeout=TIMEOUT,
...
def hf_search(query: str, kind: str = "dataset", limit: int = 15) -> str:
    ...
    try:
        r = httpx.get(f"https://huggingface.co/api/{path}", timeout=TIMEOUT,
    ...
    return "\n".join(
        ...
        f"tags={','.join((it.get('tags') or [])[:6])}" for it in items) or "(no results)"
...
def hf_info(repo_id: str, kind: str = "dataset") -> str:
    ...
    path = "datasets" if kind.startswith("data") else "models"
    try:
        r = httpx.get(f"https://huggingface.co/api/{path}/{repo_id}", timeout=TIMEOUT,
                      params={"full": "true"})
        r.raise_for_status()
        d = r.json()
    except Exception as e:
